...
static void get_blk_fw_using_dist(
    ...
    for (uint32_t idx_32x32 = 0; idx_32x32 < 4; idx_32x32++) {
        ...
        if (context_ptr->tf_32x32_block_split_flag[idx_32x32] == 0)
        {
            ...
            int weight =
                context_ptr->tf_32x32_block_error[idx_32x32] < (threshold_low << THR_SHIFT)
                ? 2
                : context_ptr->tf_32x32_block_error[idx_32x32] < (threshold_high << THR_SHIFT) ? 1 : 0;
            for (int i = 0; i < 4; ++i) {
            ...
            for (int i = 0; i < 4; ++i) {
                blk_fw[idx_32x32 * 4 + i] =
                    context_ptr->tf_16x16_block_error[idx_32x32 * 4 + i] < threshold_low
                    ? 2
                    : context_ptr->tf_16x16_block_error[idx_32x32 * 4 + i] < threshold_high ? 1 : 0;
            }
        }
    }
}
    ...
    for (uint32_t blk_row = y_b64_start_idx; blk_row < y_b64_end_idx; blk_row++) {
        for (uint32_t blk_col = x_b64_start_idx; blk_col < x_b64_end_idx; blk_col++) {
            ...
            for (int frame_index = 0;
                ...
                // ------------
#if ENHANCED_TF_3X3
                const int use_planewise_strategy = (picture_control_set_ptr_central->slice_type == I_SLICE || picture_control_set_ptr_central->noise_levels[0]  >  1.2) ? 1 : 0;
#endif
                        ...
                        for (int block_col = 0; block_col < 2; block_col++) {
                            ...
                            apply_filtering_block_plane_wise(context_ptr,
                                ...
                                apply_filtering_block(
                                    ...
                                    picture_control_set_ptr_central->altref_strength,
                                    blk_fw,
                                    is_highbd);
#endif
                        }
