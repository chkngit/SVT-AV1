...
static INLINE void highbd_get_sum_8(const uint32_t *dist, __m128i *sum_first, __m128i *sum_second) {
...
static INLINE void highbd_average_4(__m128i *output, const __m128i *sum,
                                    const __m128i *mul_constants, const int strength,
...
static INLINE void highbd_average_8(__m128i *output_0, __m128i *output_1, const __m128i *sum_0_u32,
                                    const __m128i *sum_1_u32, const __m128i *mul_constants_0,
                                    const __m128i *mul_constants_1, const int strength,
                                    const int rounding, const int weight) {
    highbd_average_4(output_0, sum_0_u32, mul_constants_0, strength, rounding, weight);
    highbd_average_4(output_1, sum_1_u32, mul_constants_1, strength, rounding, weight);
}
...
static INLINE void highbd_accumulate_and_store_8(const __m128i sum_first_u32,
                                                 const __m128i sum_second_u32, const uint16_t *pred,
...
static INLINE void highbd_read_chroma_dist_row_8(int ss_x, const uint32_t *u_dist,
                                                 const uint32_t *v_dist, __m128i *u_first,
                                                 __m128i *u_second, __m128i *v_first,
                                                 __m128i *v_second) {
    if (!ss_x) {
...
static void highbd_apply_temporal_filter_luma_8(
    ...
    assert(strength >= 0 && strength <= 14 && "invalid adjusted temporal filter strength");
    ...
    // Add chroma values
    highbd_read_chroma_dist_row_8(ss_x, u_dist, v_dist, &u_first, &u_second, &v_first, &v_second);
    ...
    highbd_average_8(&sum_row_first,
                     &sum_row_second,
                     &sum_row_first,
                     &sum_row_second,
                     &mul_first,
    ...
    for (h = 1; h < block_height - 1; ++h) {
        ...
        if (!use_whole_blk && h == block_height / 2) { weight = bottom_weight; }
        // Shift the rows up
...
static INLINE void highbd_add_luma_dist_to_8_chroma_mod(const uint32_t *y_dist, int ss_x, int ss_y,
                                                        __m128i *u_mod_fst, __m128i *u_mod_snd,
                                                        __m128i *v_mod_fst, __m128i *v_mod_snd) {
...
static void highbd_apply_temporal_filter_chroma_8(
    ...
    const uint32_t *const *neighbors_snd, int top_weight, int bottom_weight, const int *blk_fw) {
    const int rounding = (1 << strength) >> 1;
    int       weight   = top_weight;
    ...
    highbd_add_luma_dist_to_8_chroma_mod(
        y_dist, ss_x, ss_y, &u_sum_row_fst, &u_sum_row_snd, &v_sum_row_fst, &v_sum_row_snd);

    // Get modifier and store result
    if (blk_fw) {
        highbd_average_4(&u_sum_row_fst, &u_sum_row_fst, &mul_fst, strength, rounding, blk_fw[0]);
        highbd_average_4(&u_sum_row_snd, &u_sum_row_snd, &mul_snd, strength, rounding, blk_fw[1]);

        highbd_average_4(&v_sum_row_fst, &v_sum_row_fst, &mul_fst, strength, rounding, blk_fw[0]);
        highbd_average_4(&v_sum_row_snd, &v_sum_row_snd, &mul_snd, strength, rounding, blk_fw[1]);

    } else {
        highbd_average_8(&u_sum_row_fst,
                         &u_sum_row_snd,
                         &u_sum_row_fst,
                         &u_sum_row_snd,
                         &mul_fst,
                         ...
                         rounding,
                         weight);
        highbd_average_8(&v_sum_row_fst,
                         &v_sum_row_snd,
                         &v_sum_row_fst,
                         &v_sum_row_snd,
                         &mul_fst,
    ...
    for (h = 1; h < uv_block_height - 1; ++h) {
        ...
        if (blk_fw) {
            highbd_average_4(
                &u_sum_row_fst, &u_sum_row_fst, &mul_fst, strength, rounding, blk_fw[0]);
            highbd_average_4(
                &u_sum_row_snd, &u_sum_row_snd, &mul_snd, strength, rounding, blk_fw[1]);

            highbd_average_4(
                &v_sum_row_fst, &v_sum_row_fst, &mul_fst, strength, rounding, blk_fw[0]);
            highbd_average_4(
                &v_sum_row_snd, &v_sum_row_snd, &mul_snd, strength, rounding, blk_fw[1]);
...
void svt_av1_highbd_apply_temporal_filter_sse4_1(
    ...
    uint32_t *      y_dist_ptr = y_dist + 1, *u_dist_ptr = u_dist + 1, *v_dist_ptr = v_dist + 1;
    ...
    for (row = 0; row < block_height; row++) {
        for (blk_col = 0; blk_col < block_width; blk_col += 8) {
            highbd_store_dist_8(y_src_ptr + blk_col, y_pre_ptr + blk_col, y_dist_ptr + blk_col);
        }
    ...
    for (row = 0; row < chroma_height; row++) {
        for (blk_col = 0; blk_col < chroma_width; blk_col += 8) {
            ...
            highbd_store_dist_8(v_src_ptr + blk_col, v_pre_ptr + blk_col, v_dist_ptr + blk_col);
        }
